...
<div class="container c28464">
        <div class="row c28723">
            <div class="col-12 c28731 col-md-12 col-sm-12 col-xl-8 col-lg-8 mx-auto">
                <h2 data-type="header" id="ikjko3">Search Results </h2>
                <hr />
                <h2 data-type="header" id="ikjko3">Lessons</h2>
                <div class="upcoming-products mb-3">
                <?php if(!empty($lessons) && $_GET['search'] != ""):?>
                    ...
                    <?php endforeach;?>
                    <?php else:?>
                        &emsp;&nbsp;<i class="text-secondary">No Results</i>
                    <?php endif;?>
                </div>
                
                <h2 data-type="header" id="ikjko3">Users</h2>
                    <div class="card-body">
                            <ul class="list-group list-group-flush list my--3">
                <?php if(!empty($names) && $_GET['search'] != ""):?>
                <?php foreach($names as $name):?>
                                <li class="list-group-item border">
                                    <div class="row align-items-center">
                                        <div class="col-auto"><img data-type="image" src="<?=base_url('public/uploads');?>/<?= $name->profile;?>" alt="user team member profile image"
                                                    class="img-fluid rounded-circle mx-auto d-block" style="height:80px; width:80px;"/></div>
                                        <div class="col ml-2">
                                            <h4 data-type="header" class="mb-0"><?= $name->name;?></h4>
                                            <i class="text-secondary"><?= $name->bio;?></i>
                                        </div>
                                        
                                        <div class="col-auto"><a href="<?=base_url('dashboard/visit')?>?id=<?= $name->code;?>" style="text-decoration: none;"><button type="button" class="btn btn-sm btn-danger follow-btn">Visit</button></a></div>
                                    </div>
                                </li>
                <?php endforeach;?>
                            ...
                            <?php else:?>
                <i class="text-secondary"><p style="margin-top: -20px;">No Results</p></i>
                <?php endif;?>
