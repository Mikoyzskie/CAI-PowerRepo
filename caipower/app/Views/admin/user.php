    ...
    <div id="gb-main-bar-content" class="page-content collapsible-bar-1 pl-5 pr-5 pb-2">
        ...
        <div class="row c28723">
            <div class="col-12 c28731 col-md-12 col-sm-12 col-xl-8 col-lg-8 mx-auto">
                <h2 data-type="header" id="ikjko3">New Teacher / Administrator </h2>
                <hr />
                <div class="card">
                    <div class="card-body">
                        <form id="lessonform" method="post" action="<?= base_url('admin/insertUser');?>" enctype="multipart/form-data">
                        <?= csrf_field();?>
                            <div class="form-group"><label for="productName">Full Name:</label><input type="text" placeholder="John Doe" class="form-control" required name="name" value="<?= set_value('name');?>"/></div>
                            
                            <div class="form-group"><label for="exampleInputName">Email Address:</label>
                            <span class="text-danger"><?= isset($validation) ? display_error($validation,'email') : '' ?></span>
                            <input type="text" placeholder="johndoe@rocketmail.com" class="form-control" required name="email" value="<?= set_value('email');?>"/></div>
                            

                            <div class="form-group"><label for="exampleInputName">Enter Password:</label>
                            <span class="text-danger"><?= isset($validation) ? display_error($validation,'password') : '' ?></span>
                            <input type="text" placeholder="Atleast 5 character password" class="form-control" required name="password" value="<?= set_value('password');?>"/></div>
                            

                            <div class="form-group"><label for="exampleInputName">Confirm Password:</label>
                            <span class="text-danger"><?= isset($validation) ? display_error($validation,'cpassword') : '' ?></span>
                            <input type="text" placeholder="Match password" class="form-control" required name="cpassword" value="<?= set_value('cpassword');?>"/></div>
                            
                            <div class="form-group"><label for="exampleInputName">Profile picture:</label>
                            <span class="text-danger"><?= isset($validation) ? display_error($validation,'userfile') : '' ?></span>
                            <div class="input-group">
                                    <div class="input-group-prepend"><span id="inputGroupFileAddon01" class="input-group-text"><i class="fa fa-image">
                                            </i></span></div>
                                    <div class="custom-file"><input type="file" id="inputGroupFile01" aria-describedby="inputGroupFileAddon01" class="custom-file-input" name="userfile"/><label for="inputGroupFile01"
                            ...
                            <button
                                type="submit" class="btn btn-outline-secondary">Insert</button>
                        </form>
                    </div>
                </div>
